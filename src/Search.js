...
function Search(props){
    ...
    function searchInput(e){
        ...
        */
        const url = `https://api.openweathermap.org/data/2.5/weather?q=${currentValue}&appid=bae345b78e7c5ff9ce9494d7e171b4f1&units=metric`;
        fetch(url)
        ...
        .then(data=>{
            ...
            if(weather != undefined){
                ...
                const icon = `https://s3-us-west-2.amazonaws.com/s.cdpn.io/162656/${
                    weather[0]["icon"]}.svg`;
                    setCidade(`
                    <div class="estiloCidade">
                    <p>Temperatura: ${main.temp}</p>
                    <p>País: ${sys.country}</p>
                    <p>Cidade: ${name}</p>
                    <p>Condição do tempo: ${weather[0]['description']}</p>
                    <img src="${icon}" />
                    </div>
                    `);
        }
    ...
    return(
        <div className="searchWraper">
            ...
            <h2>Digite a cidade para saber a previsão do tempo</h2>
